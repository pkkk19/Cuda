...
__device__ char findFirstLetter(char *letters)
{
	for (int i = 0; i < 3; i++)
	{
		if (i == 0)
		{
			...
			if (convertCharacter(possible - 2) == letters[1] && convertCharacter(possible + 1) == letters[2])
				return possible;
		}
		else if (i == 1)
		{
			char possible = letters[1] + 2;
			if (convertCharacter(possible + 2) == letters[0] && convertCharacter(possible + 1) == letters[2])
				return possible;
		}
		else
		{
			char possible = letters[2] - 1;
			if (convertCharacter(possible + 2) == letters[0] && convertCharacter(possible - 2) == letters[1])
				return possible;
		}
	}
}
...
__device__ char findSecondLetter(char *letters)
{
	for (int i = 0; i < 3; i++)
	{
		if (i == 0)
		{
			...
			if (convertCharacter(possible - 3) == letters[1] && convertCharacter(possible - 1) == letters[2])
				return possible;
		}
		else if (i == 1)
		{
			char possible = letters[1] + 3;
			if (convertCharacter(possible + 3) == letters[0] && convertCharacter(possible - 1) == letters[2])
				return possible;
		}
		else
		{
			char possible = letters[2] + 1;
			if (convertCharacter(possible + 3) == letters[0] && convertCharacter(possible - 3) == letters[1])
				return possible;
		}
	}
}
...
__device__ char findFirstDigit(char *letters)
{
	for (int i = 0; i < 2; i++)
	{
		...
		{
			char possible = letters[0] - 2;
			if (convertNumber(possible - 2) == letters[1])
				return possible;
		}
		else
		{
			char possible = letters[1] + 2;
			if (convertNumber(possible + 2) == letters[0])
				return possible;
		}
	}
}
...
__device__ char findSecondDigit(char *letters)
{
	for (int i = 0; i < 2; i++)
	{
		if (i == 0)
		{
			...
			if (convertNumber(possible - 4) == letters[1])
				return possible;
		}
		else
		{
			char possible = letters[1] + 4;
			if (convertNumber(possible + 4) == letters[0])
				return possible;
		}
	}
}
...
__device__ char *CudaDecrypt(char *encryptedPassword)
{
	...
	printf("decrypted password of %s : %s \n", encryptedPassword, test);
}
